 ...
 * @author sz
 */
public class SpringMVCFileWriter implements IWriter {

    @Override
    public void write(Map<String, Entity> entityMap) throws Exception {
        StringBuilder dirBuffer = new StringBuilder();
        dirBuffer.append(SysUtils.formatPathFromPackage(Cache.getInstance().getSetting().getPackage(), "."));
        dirBuffer.append("\\");
        ...
        for (Entity entity : entityMap.values()) {
            ...
            data.put("moduleName", Cache.getInstance().getAdvancedSetting().getModuleName());
